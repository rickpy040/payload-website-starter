...
const LOCATIES: Row[] = [
  ...
  },
  {
    ...
    googleMapsUrl:
      'https://maps.google.com/maps?daddr=Frederik+van+Eedenplein+5615+Eindhoven&hl=nl',
  },
  {
    name: 'Evoluon',
    slug: 'evoluon',
  ...
  },
  {
    ...
    leverancier: 'WPS',
    googleMapsUrl:
      'https://maps.google.com/maps?daddr=Dag+Hammarskj%C3%B6ldhof+1314+WK+Almere&hl=nl',
  },
  {
    name: 'Europoint',
    slug: 'europoint',
  ...
  },
  {
    ...
    leverancier: 'IP Parking',
    googleMapsUrl:
      'https://maps.google.com/maps?daddr=Plantage+Kerklaan+38+1018+CZ+Amsterdam&hl=nl',
  },
  {
    name: 'Cruquius',
    slug: 'cruquius',
  ...
  },
  {
    ...
    leverancier: null,
    googleMapsUrl:
      'https://maps.google.com/maps?daddr=Willem+van+Arenbergstraat+4+6515+AT+Nijmegen&hl=nl',
  },
  {
    name: 'Pathé Ede',
    slug: 'pathe-ede',
    ...
    leverancier: null,
    googleMapsUrl:
      'https://maps.google.com/maps?daddr=Laan%20der%20Verenigde%20Naties%20150+6716%20JE%20Ede+13&hl=nl',
  },
  {
...
const SEEDED: Record<string, Record<string, unknown>> = {
  'philips-stadion': {
    ...
    spotsTotal: 128,
    soort: 'Parkeerterrein',
    'address.street': 'Frederiklaan',
    'address.huisnummer': '10',
  },
  'dll-parkeerdek': { spotsTotal: 92 },
  'philips-bedrijfsschool': {
    spotsTotal: 138,
    'address.street': 'Kastanjelaan',
    'address.huisnummer': '400',
  },
}
...
const normName = (s: string) =>
  s
    .toLowerCase()
    .replace(/^(parkingyou|parking)\s+/, '')
    .replace(/[^a-z0-9]/g, '')

export async function up({ payload }: MigrateUpArgs): Promise<void> {
  // Deliberately not passing the migration's `req`: when a local API operation
  // fails (e.g. validation on a published location without a price), Payload
  // rolls back the whole transaction on that req, silently undoing everything
  // before it. Each operation gets its own transaction instead; the import is
  // idempotent, so a partial run is completed by running it again.
  ...
  for (const slug of new Set(LOCATIES.map((l) => l.stad))) {
    ...
    const doc =
      ...
      (await payload.create({
        collection: 'steden',
        data: {
          naam: STAD_NAAM[slug] ?? slug,
          slug,
          isRegio: slug.startsWith('regio-'),
          _status: 'published',
        } as never,
        ...common,
      }))
  ...
  for (const row of LOCATIES) {
    const existing =
      (row.oudeId != null ? byOudeId.get(row.oudeId) : undefined) ??
      bySlug.get(row.slug) ??
      byName.get(normName(row.name))
    ...
    if (!existing) {
      try {
        ...
        skipped++
        payload.logger.warn(`import: ${row.slug} not created: ${(err as Error).message}`)
      }
      continue
    }
  ...
  payload.logger.info(
    `import: ${created} created as draft, ${updated} filled in, ${skipped} unchanged`,
  )
}
